...
class CRUDParticipants(CRUDBase[Participants, ParticipantsInDB, UpdateParticipants]):
    ...
    def get_active_participant(self, db: Session, participant_code: str) -> Optional[Participants]:
        logger.info(f"get_active_participant is triggered for participant {participant_code}")
        try:
            return db.query(Participants).filter(and_(Participants.participant_code == participant_code, Participants.is_active == True)).first()
        except Exception as e:
            logger.exception(f"Error occured in get_active_participant : {e}")
